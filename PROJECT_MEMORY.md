...
- The extension's formal share action uses the same R2-backed service as the website. Guest shares, including all current extension shares, have a fixed 1-day expiry; signed-in website users can choose 1/7/30 days or persistent retention. Optional text watermarks and download presentation remain one implementation across both surfaces. Existing Wrangler modules remain for compatibility with older records and focused regression coverage, not as a parallel sidebar publishing path.
