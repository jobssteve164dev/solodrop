...
const translations: Record<SupportedLocale, Strings> = {
  ...
  'zh-cn': {
    ...
    temporaryProvisioningUnavailable: 'Cloudflare 多次尝试后仍无法创建临时预览。文件没有发布，请稍等片刻后重新分享。', creatingShortLink: '正在生成并检查短链接…', shortLinkFallback: '预览已可访问，但短链服务暂时不可用；已改为复制验证过的原始链接。', managedTemporaryMeta: '短链接 · 临时预览', managedPersistentMeta: '短链接 · 长期预览', views: '{count} 次访问'
  }
};
