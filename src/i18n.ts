...
export interface Strings {
  ...
  views: string;
  saveShareCard: string; shareCardSaved: string;
  shareCardScanHint: string; shareCardExpiresAt: string;
...
const translations: Record<SupportedLocale, Strings> = {
  ...
  'zh-cn': {
    ...
    temporaryProvisioningUnavailable: 'Cloudflare 多次尝试后仍无法创建临时预览。文件没有发布，请稍等片刻后重新分享。', creatingShortLink: '正在生成并检查短链接…', shortLinkFallback: '预览已可访问，但短链服务暂时不可用；已改为复制验证过的原始链接。', managedTemporaryMeta: '可控分享链接', managedPersistentMeta: '长期分享链接', views: '{count} 次访问', saveShareCard: '保存分享名片', shareCardSaved: '分享名片已保存。', shareCardScanHint: '长按识别二维码', shareCardExpiresAt: '链接有效期至 {date}',
