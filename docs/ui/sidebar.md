...
5. Recent previews for reopening delivered results. Share metadata follows the user's VS Code Settings Sync account across devices without syncing local file paths; bounded shares show whether their selected 1/7/30-day lifetime is still active, and expired entries can be shared again.
...
- Re-sharing an expired preview reuses the original local file when available. On another device, or when the original was an operating-system drop, the user chooses the source file again and continues through the same safety, lifetime, watermark and download controls.
